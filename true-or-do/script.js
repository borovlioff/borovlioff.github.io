...
const partnes = JSON.parse(localStorage.getItem('partnes')) || [];
...
function getRandomPartne() {
  ...
  const randomIndex = Math.floor(Math.random() * partnes.length);
  return partnes[randomIndex];
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  addUserButton.addEventListener('click', () => {
    ...
    if (username !== "") {
      partnes.push(username);
      usernameInput.value = ''; // Очистка поля ввода
      updateParticipantList(); // Обновление списка участников
  ...
  truthButton.addEventListener('click', () => {
    ...
    const randomTruth = getRandomElement(questions);
    patner.textContent = getRandomPartne();
    output.textContent = randomTruth;
  });
  ...
  dareButton.addEventListener('click', () => {
    ...
    const randomDare = getRandomElement(dare);
    patner.textContent = getRandomPartne();
    output.textContent = randomDare;
  });
